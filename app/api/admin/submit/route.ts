...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await resend.emails.send({
      ...
      to: email,
      subject: `Your ${(application?.scheme || scheme).toUpperCase()} application has been submitted to HMRC - ${application?.company_name}`,
      html: `
        <div style="font-family: Georgia, serif; max-width: 560px; margin: 0 auto; padding: 40px 20px; color: #1a1a18;">
          ...
          <p style="font-size: 15px; line-height: 1.6; color: #555; margin-bottom: 16px;">
            We have submitted your ${(application?.scheme || scheme).toUpperCase()} advance assurance application to HMRC's Venture Capital Reliefs team on behalf of <strong>${application?.company_name}</strong>.
          </p>
          <p style="font-size: 15px; line-height: 1.6; color: #555; margin-bottom: 16px;">
